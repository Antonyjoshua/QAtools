...
export function usePomodoro() {
  const pomodoro = useQuickTimerStore((s) => s.pomodoro);
  const pomodoroSettings = useQuickTimerStore((s) => s.pomodoroSettings);
  const updateSettings = useQuickTimerStore((s) => s.updatePomodoroSettings);
  const start = useQuickTimerStore((s) => s.pomodoroStart);
  const pause = useQuickTimerStore((s) => s.pomodoroPause);
  const skip = useQuickTimerStore((s) => s.pomodoroSkip);
  const resetCycle = useQuickTimerStore((s) => s.pomodoroResetCycle);
  const checkCompletion = useQuickTimerStore((s) => s.checkPomodoroCompletion);
