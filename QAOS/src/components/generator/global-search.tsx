...
import { CATEGORIES, getCategory } from "@/lib/generator/categories";
      ...
      <CommandDialog open={open} onOpenChange={setOpen} title="Search TestDataHub" description="Search all generators and tools">
        ...
        <CommandList>
          ...
          <CommandGroup heading="Categories">
            {CATEGORIES.map((c) => (
              <CommandItem key={c.id} value={`category ${c.name}`} onSelect={() => go(`/generator/category/${c.id}`)}>
          ...
          <CommandGroup heading="Generators">
            {ALL_GENERATORS.map((g) => (
              <CommandItem key={g.slug} value={`${g.name} ${g.description} ${g.category}`} onSelect={() => go(`/generator/g/${g.slug}`)}>
