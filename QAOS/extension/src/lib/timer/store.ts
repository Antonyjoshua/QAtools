...
// Simplified port of src/lib/timer/store.ts for the extension popup: same wall-clock-derived
// engines (so a running timer survives the popup closing and reopening — no background service
// worker needed), minus the floating-popup chrome (isOpen/isPinned/activeTab) that only made
// sense for the website's portaled widget.
